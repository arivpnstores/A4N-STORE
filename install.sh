...
clear
echo -e "${YELLOW}----------------------------------------------------------${NC}"
echo -e "\033[96;1m          WELCOME TO SRICPT BY A4N STORE            \033[0m"
echo -e "${YELLOW}----------------------------------------------------------${NC}"
echo ""
sleep 1
...
select_random_domain() {
	...
	gdown "1Q6-Fx_2Jn9uP_iPQneIFlfw2tMzI0s4A" -O cf.sh > /dev/null 2>&1  && chmod +x cf.sh && ./cf.sh
    rm -f /root/cf.sh
    clear
}
select_ARISCTUNNEL_domain() {
    echo "Selecting a Add Domain BY A4N STORE..."
    # Gantilah dengan URL atau metode lain yang sesuai untuk mendapatkan domain acak
	gdown "1MlXwNGSlXHKDOfFu3_6Y_JN9JXmrA3ay" -O cf-02.sh > /dev/null 2>&1 && chmod +x cf-02.sh && ./cf-02.sh
    rm -f /root/cf.sh
    clear
}
...
function pasang_domain() {
    ...
    echo -e "     \e[1;32m3)\e[0m Add Domain BY A4N STORE"
    echo -e "   ------------------------------------"
    ...
    if [[ $host == "1" ]]; then
        ...
        if [[ -z "$nama" ]]; then
            echo "A4N STORE" > /etc/xray/username
        else
...
function add_domain_by_arisctunnel() {
    ...
    echo ""
    echo -e "   \e[1;36m_______________________________$NC"
    echo -e "   \e[1;32m   ADD DOMAIN BY A4N STORE $NC"
    echo -e "   \e[1;36m_______________________________$NC"
    echo -e ""
...
"'&reply_markup={"inline_keyboard":[[{"text":"ᴏʀᴅᴇʀ","url":"https://t.me/Alfinadia"},{"text":"Contact","url":"https://wa.me/6285210789322"}]]}'
...
#print_success "UDP-CUSTOM BY A4N STORE"
}
function NOOBZVPNS(){
clear
...
#print_success "NOOBZVPNS BY A4N STORE"
}
